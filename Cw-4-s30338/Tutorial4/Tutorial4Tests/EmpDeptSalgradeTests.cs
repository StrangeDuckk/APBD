...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var deptsChicago = depts.Where(d => d.Loc.Equals("CHICAGO")).Select(d => d.DeptNo).ToList(); //wszystkie deptNo ktore sa w Chicago
        List<Emp> result = emps.Where(e => deptsChicago.Contains(e.DeptNo)).ToList(); //sprawdzenie czy zawieraja
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Select(e => new { e.EName, e.Sal}).ToList(); //typ anonimowy
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(e => e.Comm is not null)
                         .SelectMany(e => e.EName,(e,c) => new { e.EName, e.Comm })
                         .ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(e => e.Sal > 
                                            emps.Where(s => s.DeptNo == e.DeptNo)//subquery
                                                .Average(s => s.Sal))
                         .Select(e => e.EName)
                         .ToList(); 
